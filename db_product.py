...
class Db:
  ...
  def insertData(self,id_produto,nome,valor):
     try:
        self.openConnection()
        cursor= self.connection.cursor()
        insertQuery= "INSERT INTO produtos (id_produto,nome,valor) VALUES(%s,%s, %s)"
        recordInsert= (id_produto,nome,valor)
  ...
  def updateData(self, id_produto, nome, valor):
    try:
        self.openConnection()
        cursor = self.connection.cursor()
        
        print("Register before the Upgrade")
        selectedQuery = """SELECT * FROM produtos WHERE id_produto = %s"""
        cursor.execute(selectedQuery, (id_produto,))
        record_before = cursor.fetchone()

        sqlUpdate = """UPDATE produtos SET nome = %s, valor = %s WHERE id_produto = %s"""
        cursor.execute(sqlUpdate, (nome, valor, id_produto))
        self.connection.commit()
        count = cursor.rowcount
  ...
  def deleteData(self, id_produto):
    try:
        self.openConnection()
        cursor = self.connection.cursor()

        deleteQuery = """DELETE from produtos WHERE id_produto = %s"""
        cursor.execute(deleteQuery, (id_produto,))
        self.connection.commit()
        count = cursor.rowcount()
